...
async function run() {
  try {
    ...
    app.get("/products-amount", async (req, res) => {
      const totalProduct = await productCollection.countDocuments();
      res.send({ totalItem: totalProduct });
    });

    app.post("/cart",async (req,res) => {
      console.log(req.body)
      const storedItems = req.body
      const idWithObjectId=storedItems.map(id=>new ObjectId(id))
      console.log(idWithObjectId)

      const query={
        _id : {$in:idWithObjectId}
      }
